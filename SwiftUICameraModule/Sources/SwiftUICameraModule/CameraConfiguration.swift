...
/// CameraConfiguration, media capture configuration object
public class CameraConfiguration {
    ...
    public func avcaptureSettingsDictionary(service: CameraService, sampleBuffer: CMSampleBuffer? = nil, pixelBuffer: CVPixelBuffer? = nil) -> [String: Any]? {
        self.options
    }
}
...
/// CameraVideoConfiguration, video capture configuration object
public class CameraVideoConfiguration: CameraConfiguration {
    ...
    public var bitRate: Int = CameraVideoConfiguration.VideoBitRateDefault

    /// Dimensions for video output, AV dictionary keys AVVideoWidthKey, AVVideoHeightKey
    public var dimensions: CGSize?

    /// Output aspect ratio automatically sizes output dimensions, `active` indicates CameraVideoConfiguration.preset or CameraVideoConfiguration.dimensions
    public var aspectRatio: AspectRatio = .active

    /// Video output transform for display
    public var transform: CGAffineTransform = .init(rotationAngle: CGFloat.pi / 2)//identity
    ...
    public var maximumCaptureDuration: CMTime? = CMTime(seconds: 30, preferredTimescale: 1)
    ...
    override public func avcaptureSettingsDictionary(service: CameraService, sampleBuffer: CMSampleBuffer? = nil, pixelBuffer: CVPixelBuffer? = nil) -> [String: Any]? {
...
/// CameraAudioConfiguration, audio capture configuration object
public class CameraAudioConfiguration: CameraConfiguration {
    ...
    public var bitRate: Int = CameraAudioConfiguration.AudioBitRateDefault
    ...
    override public func avcaptureSettingsDictionary(service: CameraService, sampleBuffer: CMSampleBuffer? = nil, pixelBuffer: CVPixelBuffer? = nil) -> [String: Any]? {
        ...
        if let sampleRate = self.sampleRate {
            config[AVSampleRateKey] = sampleRate == 0 ? NSNumber(value: CameraAudioConfiguration.AudioSampleRateDefault) : NSNumber(value: sampleRate)
        } else {
            config[AVSampleRateKey] = NSNumber(value: CameraAudioConfiguration.AudioSampleRateDefault)
        }

        if let channels = self.channelsCount {
            config[AVNumberOfChannelsKey] = channels == 0 ? NSNumber(integerLiteral: CameraAudioConfiguration.AudioChannelsCountDefault) : NSNumber(integerLiteral: channels)
        } else {
            config[AVNumberOfChannelsKey] = NSNumber(integerLiteral: CameraAudioConfiguration.AudioChannelsCountDefault)
        }
...
// MARK: - CameraVideoDelegate

/// Video delegate, provides updates on video related recording and capture functionality.
/// All methods are called on the main queue with the exception of Camera:renderToCustomContextWithSampleBuffer:onQueue.
public protocol CameraVideoDelegate: AnyObject {
    
    func Camera(_ Camera: CameraService, didCompleteClip clip: CameraClip, inSession session: CameraSession)
